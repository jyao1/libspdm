...
libspdm_return_t libspdm_auth_get_response_set_auth_policy_done(
    ...
    void *response)
{
    const spdm_auth_set_auth_policy_request_t *spdm_auth_request;
    spdm_auth_set_auth_policy_done_response_t *spdm_auth_response;
    ...
    LIBSPDM_ASSERT(spdm_auth_request->header.request_response_code == SPDM_AUTH_SET_AUTH_POLICY);
    ...
    if (request_size < sizeof(spdm_auth_set_auth_policy_request_t)) {
        return libspdm_auth_generate_error_response(
            spdm_context,
            SPDM_AUTH_ERROR_CODE_INVALID_REQUEST,
            response_size, response);
    }
    /* Guard against integer overflow before using num_of_policies in arithmetic */
    if (spdm_auth_request->policy_list.num_of_policies > LIBSPDM_AUTH_MAX_POLICY_LIST_COUNT) {
        return libspdm_auth_generate_error_response(
            spdm_context,
            SPDM_AUTH_ERROR_CODE_INVALID_REQUEST,
            response_size, response);
    ...
    if (request_size < sizeof(spdm_auth_set_auth_policy_request_t) +
                       spdm_auth_request->policy_list.num_of_policies *
    ...
    switch (spdm_auth_request->set_auth_policy_op) {
    case SPDM_AUTH_SET_AUTH_POLICY_OP_LOCK:
    case SPDM_AUTH_SET_AUTH_POLICY_OP_UNLOCK:
    ...
    ret = libspdm_auth_device_set_auth_policy (
        spdm_context, session_id, has_auth,
        spdm_auth_request->set_auth_policy_op,
        sizeof(spdm_auth_policy_list_t) +
        spdm_auth_request->policy_list.num_of_policies *
        sizeof(spdm_auth_policy_struct_for_dsp0289_t),
        &spdm_auth_request->policy_list,
        &operation_failed);
    if (!ret) {
        if (operation_failed) {
            return libspdm_auth_generate_error_response(
    ...
    LIBSPDM_ASSERT(*response_size >= sizeof(spdm_auth_set_auth_policy_done_response_t));
    *response_size = sizeof(spdm_auth_set_auth_policy_done_response_t);
