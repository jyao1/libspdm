    ...
    size_t *response_size)
{
    ...
    ctx = (libspdm_context_t *)spdm_context;

    /* Determine if large request format (SPDM 1.4+) is used.
     * In LARGE format the layout after the fixed header is:
     *   [0..1]  skipped uint16_t placeholder  (NOT the payload length)
     *   [2..5]  large_request_len  uint32_t   (= actual auth payload size)
     *   [6+]    auth record
     * In NON-LARGE format:
     *   [0..1]  payload_length uint16_t
     *   [2+]    auth record
     */
    bool use_large_req = (spdm_request->header.param1 &
                          SPDM_VENDOR_DEFINED_REQUEST_LARGE_REQ) != 0;
    size_t auth_header_size;
    uint32_t auth_payload_length;

    if (use_large_req) {
        auth_header_size = sizeof(spdm_auth_vendor_defined_request_t) + sizeof(uint32_t);
        if (request_size < auth_header_size + sizeof(spdm_auth_record_t)) {
            return LIBSPDM_STATUS_INVALID_MSG_SIZE;
        }
        auth_payload_length = libspdm_read_uint32(
            (const uint8_t *)request + sizeof(spdm_auth_vendor_defined_request_t));
    } else {
        auth_header_size = sizeof(spdm_auth_vendor_defined_request_t);
        if (request_size < auth_header_size + sizeof(spdm_auth_record_t)) {
            return LIBSPDM_STATUS_INVALID_MSG_SIZE;
        }
        auth_payload_length = spdm_request->auth_vendor_header.payload_length;
    }

    /* For LARGE format responses, an extra uint32_t large_response_len precedes the auth record. */
    size_t rsp_extra = use_large_req ? sizeof(uint32_t) : 0;

    LIBSPDM_ASSERT (*response_size > sizeof(spdm_auth_vendor_defined_response_t) +
                                     rsp_extra + sizeof(spdm_auth_record_t));
    ...
    if (auth_payload_length >
        request_size - auth_header_size) {
        return LIBSPDM_STATUS_INVALID_MSG_FIELD;
    }
    if (auth_payload_length <
    ...
    header_with_auth = (void *)((uint8_t *)request + auth_header_size);
    header_no_auth = (void *)header_with_auth;
    auth_failed = false;
    if (header_with_auth->auth_record_type == SPDM_AUTH_RECORD_TYPE_AUTH_MESSAGE_WITH_AUTH) {
        ...
        if (req_payload_len > auth_payload_length -
                              (sizeof(spdm_auth_record_t) +
        ...
        sign_header = (void *)ctx->auth_sign_msg_scratch_buffer;
        ...
        libspdm_copy_mem(
            sign_header + 1,
            sizeof(ctx->auth_sign_msg_scratch_buffer) - sizeof(spdm_auth_record_sign_header_t),
        ...
        ret = libspdm_auth_device_requester_data_verify (
            ...
            ctx->auth_sign_msg_scratch_buffer, req_payload_len + sizeof(spdm_auth_record_sign_header_t),
...
build_final_response:
    ...
    if (use_large_req) {
        /* Set LARGE_RESP flag; payload_length field stays 0 (reserved) */
        spdm_response->header.param1 = SPDM_VENDOR_DEFINED_RESPONSE_LARGE_RESP;
        /* Guard against overflow before narrowing cast to uint32_t */
        if (rsp_payload_len > UINT32_MAX - sizeof(spdm_auth_record_t)) {
            return LIBSPDM_STATUS_INVALID_MSG_SIZE;
        }
        libspdm_write_uint32((uint8_t *)response + sizeof(spdm_auth_vendor_defined_response_t),
                             (uint32_t)(sizeof(spdm_auth_record_t) + rsp_payload_len));
        ...
        *response_size = sizeof(spdm_auth_vendor_defined_response_t) + sizeof(uint32_t) +
            sizeof(spdm_auth_record_t) + header_no_auth->payload_len;
    } else {
        /* Guard against overflow before narrowing cast to uint16_t */
        if (rsp_payload_len > UINT16_MAX - sizeof(spdm_auth_record_t)) {
            return LIBSPDM_STATUS_INVALID_MSG_SIZE;
        }
