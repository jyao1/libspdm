    ...
    void *response)
{
    ...
    bool is_continue;
    spdm_auth_usas_saved_state_t saved_state;
    ...
    is_continue = (spdm_auth_request->attributes &
                   SPDM_AUTH_START_AUTH_ATTRIBUTES_CONTINUE) != 0;
    if (is_continue) {
        /* Continue requires a saved USAS for this credential. */
        if (!libspdm_auth_device_has_saved_usas(spdm_context, session_id,
        ...
        if (!libspdm_auth_device_load_usas(spdm_context, session_id,
                                            spdm_auth_request->credential_id,
                                            &saved_state)) {
            return libspdm_auth_generate_error_response(
                spdm_context,
    ...
    *response_size = sizeof(spdm_auth_start_auth_rsp_response_t);
    ...
    /* Start at SavedSequenceNumber when continuing; otherwise start at 1. */
    if (is_continue) {
        session_info->auth.usap.sequence_number = saved_state.saved_sequence_number;
        session_info->auth.usap.saved_sequence_number = saved_state.saved_sequence_number;
        /* The saved USAS is now active; erase it from persistent storage.
         * Per DSP0289 the target SHOULD keep the state until at least one
         * successfully authorized message, but erasing now is also conformant. */
