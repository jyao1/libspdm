...
libspdm_return_t libspdm_auth_end_auth(
    ...
    uint32_t *saved_sequence_number_out)
{
    libspdm_context_t *spdm_context;
    libspdm_session_info_t *session_info;
    libspdm_return_t status;
    spdm_auth_end_auth_request_t spdm_auth_request;
    ...
    spdm_auth_request.header.request_response_code = SPDM_AUTH_END_AUTH;
    spdm_auth_request.credential_id = credential_id;
    spdm_auth_request.attributes =
        (persist_method & SPDM_AUTH_END_AUTH_ATTRIBUTES_PERSIST_METHOD_MASK);
    spdm_auth_request_size = sizeof(spdm_auth_end_auth_request_t);
    ...
    } else if (spdm_auth_response.header.request_response_code != SPDM_AUTH_END_AUTH_RSP) {
        status = LIBSPDM_STATUS_INVALID_MSG_FIELD;
        goto receive_done;
    }
    if (spdm_auth_response_size < sizeof(spdm_auth_end_auth_rsp_response_t)) {
        status = LIBSPDM_STATUS_INVALID_MSG_SIZE;
        goto receive_done;
    }
    ...
    /* Return the SavedSequenceNumber to the caller when persisting. */
    if (persist_method != SPDM_AUTH_END_AUTH_ATTRIBUTES_PERSIST_METHOD_ERASE) {
        if (saved_sequence_number_out != NULL) {
            *saved_sequence_number_out =
