...
} libspdm_auth_select_auth_version_rsp_response_max_t;
#pragma pack()
    ...
    uint8_t auth_version)
{
    ...
    spdm_auth_select_auth_version_request_t spdm_auth_request;
    size_t spdm_auth_request_size;
    libspdm_auth_select_auth_version_rsp_response_max_t spdm_auth_response;
    ...
    spdm_auth_request.header.request_response_code = SPDM_AUTH_SELECT_AUTH_VERSION;
    spdm_auth_request.auth_version = auth_version;
    spdm_auth_request_size = sizeof(spdm_auth_select_auth_version_request_t);
    ...
    } else if (spdm_auth_response.header.request_response_code != SPDM_AUTH_SELECT_AUTH_VERSION_RSP) {
        status = LIBSPDM_STATUS_INVALID_MSG_FIELD;
        goto receive_done;
    }
    if (spdm_auth_response_size < sizeof(spdm_auth_select_auth_version_rsp_response_t)) {
        status = LIBSPDM_STATUS_INVALID_MSG_SIZE;
        goto receive_done;
    }

    session_info->auth.auth_version = auth_version << SPDM_AUTH_VERSION_NUMBER_SHIFT_BIT;
