    ...
    void *rsp_data)
{
    ...
    /* req_size is stored as uint32_t in auth record header — reject if it would truncate */
    ...
    if (need_auth) {
        ...
        header_with_auth = (void *)spdm_context->auth_req_scratch_buffer;
        header_with_auth->auth_record_type = SPDM_AUTH_RECORD_TYPE_AUTH_MESSAGE_WITH_AUTH;
        ...
        sign_header = (void *)spdm_context->auth_sign_msg_scratch_buffer;
        ...
        libspdm_copy_mem(
            sign_header + 1,
            sizeof(spdm_context->auth_sign_msg_scratch_buffer) - sizeof(spdm_auth_record_sign_header_t),
            req_data,
            req_size
            );
        ret = libspdm_auth_device_requester_data_sign (
            ...
            spdm_context->auth_sign_msg_scratch_buffer, req_size + sizeof(spdm_auth_record_sign_header_t),
            ptr, &signature_size
            );
        if (!ret) {
            return LIBSPDM_STATUS_INVALID_STATE_LOCAL;
        ...
        *(uint32_t *)ptr = (uint32_t)req_size; /* msg_to_auth_payload_len */
        ...
        libspdm_copy_mem(ptr,
                         (size_t)header_with_auth + sizeof(spdm_context->auth_req_scratch_buffer) - (size_t)ptr,
                         req_data,
                         req_size
                         );
        final_req_size = sizeof(spdm_auth_record_t) + header_with_auth->payload_len;
    } else {
        header_no_auth = (void *)spdm_context->auth_req_scratch_buffer;
        ...
        libspdm_copy_mem(header_no_auth + 1,
                         sizeof(spdm_context->auth_req_scratch_buffer) - sizeof(spdm_auth_record_t),
                         req_data,
                         req_size
                         );
    ...
    rsp_vendor_id_len = 2;
    rsp_dsp_number = 289;
    final_rsp_size = sizeof(spdm_context->auth_rsp_scratch_buffer);
    status = libspdm_vendor_send_request_receive_response (spdm_context, &session_id,
                                                           SPDM_REGISTRY_ID_DMTF_DSP,
                                                           2, &dsp_number,
                                                           (uint32_t)final_req_size,
                                                           spdm_context->auth_req_scratch_buffer,
                                                           &rsp_standard_id,
                                                           &rsp_vendor_id_len, &rsp_dsp_number,
                                                           (uint32_t *)&final_rsp_size,
                                                           spdm_context->auth_rsp_scratch_buffer
                                                           );
    ...
    header_no_auth = (void *)spdm_context->auth_rsp_scratch_buffer;
