...
} libspdm_auth_ownership_taken_response_max_t;
#pragma pack()
    ...
    void *context, uint32_t session_id)
{
    ...
    spdm_auth_take_ownership_request_t spdm_auth_request;
    size_t spdm_auth_request_size;
    libspdm_auth_ownership_taken_response_max_t spdm_auth_response;
    ...
    spdm_auth_request.header.request_response_code = SPDM_AUTH_TAKE_OWNERSHIP;
    spdm_auth_request_size = sizeof(spdm_auth_take_ownership_request_t);
    ...
    } else if (spdm_auth_response.header.request_response_code != SPDM_AUTH_OWNERSHIP_TAKEN) {
        status = LIBSPDM_STATUS_INVALID_MSG_FIELD;
        goto receive_done;
    }
    if (spdm_auth_response_size < sizeof(spdm_auth_ownership_taken_response_t)) {
        status = LIBSPDM_STATUS_INVALID_MSG_SIZE;
        goto receive_done;
    }
