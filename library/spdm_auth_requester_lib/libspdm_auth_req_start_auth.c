...
libspdm_return_t libspdm_auth_start_auth(
    ...
    bool is_continue,
    uint32_t continue_sequence_number)
{
    ...
    spdm_auth_request.attributes =
        is_continue ? SPDM_AUTH_START_AUTH_ATTRIBUTES_CONTINUE : 0;
    ...
    /* Start at the caller-supplied saved sequence number on continuation. */
    session_info->auth.usap.sequence_number =
        is_continue ? continue_sequence_number : 1;
