...
} libspdm_auth_cred_id_params_response_max_t;
#pragma pack()
    ...
    void *cred_params)
{
    ...
    spdm_auth_get_cred_id_params_request_t spdm_auth_request;
    size_t spdm_auth_request_size;
    libspdm_auth_cred_id_params_response_max_t spdm_auth_response;
    ...
    spdm_auth_request.header.request_response_code = SPDM_AUTH_GET_CRED_ID_PARAMS;
    spdm_auth_request.credential_id = credential_id;
    spdm_auth_request_size = sizeof(spdm_auth_get_cred_id_params_request_t);
    ...
    } else if (spdm_auth_response.header.request_response_code != SPDM_AUTH_CRED_ID_PARAMS) {
        status = LIBSPDM_STATUS_INVALID_MSG_FIELD;
        goto receive_done;
    }
    if (spdm_auth_response_size < sizeof(spdm_auth_cred_id_params_response_t)) {
        status = LIBSPDM_STATUS_INVALID_MSG_SIZE;
        goto receive_done;
    }
    /* Guard against integer overflow before using credential_data_size in arithmetic */
    if (spdm_auth_response.cred_params.credential_data_size > LIBSPDM_AUTH_MAX_CREDENTIAL_DATA_SIZE) {
        status = LIBSPDM_STATUS_INVALID_MSG_FIELD;
        goto receive_done;
    }
    if (spdm_auth_response_size < sizeof(spdm_auth_cred_id_params_response_t) +
                                  spdm_auth_response.cred_params.credential_data_size) {
        status = LIBSPDM_STATUS_INVALID_MSG_SIZE;
        goto receive_done;
    }
    if (spdm_auth_response.cred_params.credential_id != credential_id) {
        status = LIBSPDM_STATUS_INVALID_MSG_FIELD;
        goto receive_done;
    }
    ...
    *cred_params_size = sizeof(spdm_auth_credential_struct_t) +
                        spdm_auth_response.cred_params.credential_data_size;

    *cred_attributes = spdm_auth_response.cred_attributes;
    libspdm_copy_mem(cred_params,
                     *cred_params_size,
                     &spdm_auth_response.cred_params,
