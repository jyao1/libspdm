...
int32_t libspdm_x509_compare_date_time(const void *date_time1, const void *date_time2)
{
    ...
    if (ASN1_TIME_diff(&days, &secs, date_time1, date_time2) == 0) {
        return 0;
    }

    if (days > 0 || (days == 0 && secs > 0)) {
        return -1;
    } else if (days < 0 || (days == 0 && secs < 0)) {
        return 1;
    }
    return 0;
}
...
static bool allocate_key_context_with_dup(EVP_PKEY *pkey, void **context)
{
    ...
    /* Increment reference (X509_get_pubkey returns a reference that shouldn't outlive X509) */
...
bool libspdm_sm2_get_public_key_from_x509(const uint8_t *cert, size_t cert_size,
                                          void **sm2_context)
{
    /* SM2 is not supported by AWS-LC */
    return false;
}
                                   ...
                                   EVP_PKEY *public_key)
{
    ...
    /*get subject name from req_info - parse as X509_REQ for AWS-LC compatibility*/
    {
        const unsigned char *tmp_ptr = req_info;
        X509_REQ *tmp_req = d2i_X509_REQ(NULL, &tmp_ptr, (long)req_info_len);
        if (tmp_req) {
            X509_REQ_set_subject_name(req, X509_REQ_get_subject_name(tmp_req));
...
/* AWS-LC does not have EVP_PKEY_dup; use up_ref instead */
static EVP_PKEY *evp_pkey_dup_via_upref(EVP_PKEY *pkey)
{
    if (pkey == NULL) {
        return NULL;
    }
    ...
    void *base_cert)
{
    ...
    if (asym_nid != 0) {
        ...
        case LIBSPDM_CRYPTO_NID_RSAPSS4096: {
            ...
            private_key = evp_pkey_dup_via_upref(rsa_ctx->evp_pkey);
            public_key = evp_pkey_dup_via_upref(rsa_ctx->evp_pkey);
        ...
        case LIBSPDM_CRYPTO_NID_SM2_DSA_P256: {
            ...
            EVP_PKEY_free(private_key);
            EVP_PKEY_free(public_key);
            private_key = evp_pkey_dup_via_upref(sm2_pkey);
            public_key = evp_pkey_dup_via_upref(sm2_pkey);
            if (private_key == NULL || public_key == NULL) {
                goto free_all;
            }
        ...
        case LIBSPDM_CRYPTO_NID_EDDSA_ED448:
            ...
            private_key = evp_pkey_dup_via_upref(*(EVP_PKEY **)context);
            public_key = evp_pkey_dup_via_upref(*(EVP_PKEY **)context);
            if (private_key == NULL || public_key == NULL) {
                goto free_all;
            }
            hash_nid = LIBSPDM_CRYPTO_NID_NULL;
    ...
    if (pqc_asym_nid != 0) {
        ...
        case LIBSPDM_CRYPTO_NID_SLH_DSA_SHAKE_256F: {
            ...
            EVP_PKEY_free(private_key);
            EVP_PKEY_free(public_key);
            private_key = evp_pkey_dup_via_upref(pqc_ctx->evp_pkey);
            public_key = evp_pkey_dup_via_upref(pqc_ctx->evp_pkey);
            if (private_key == NULL || public_key == NULL) {
                goto free_all;
            }
            break;
