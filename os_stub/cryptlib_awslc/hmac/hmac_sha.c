...
/** @file
 * HMAC-SHA256/384/512 Wrapper Implementation over AWS-LC.
 **/
                     ...
                     const uint8_t *key, size_t key_size)
{
    ...
    if (HMAC_Init_ex((HMAC_CTX *)hmac_md_ctx, key, key_size, md, NULL) != 1) {
        return false;
    }
...
bool hmac_md_final(void *hmac_md_ctx, uint8_t *hmac_value)
{
    ...
    if (HMAC_Final((HMAC_CTX *)hmac_md_ctx, hmac_value, &len) != 1) {
        return false;
    }
                 ...
                 uint8_t *hmac_value)
{
    ...
    result = HMAC(md, key, (int)key_size, (const unsigned char *)data,
                  data_size, hmac_value, &len);
    if (result == NULL) {
        return false;
    }
