...
/**
 * Get AES-GCM EVP_CIPHER by key size
 * @param key_size Key size in bytes (16, 24, or 32)
 * @return EVP_CIPHER pointer or NULL if invalid key size
 */
static const EVP_CIPHER *get_aes_gcm_cipher(size_t key_size)
{
    switch (key_size) {
    case 16:
                                  ...
                                  uint8_t *data_out, size_t *data_out_size)
{
    ...
    cipher = get_aes_gcm_cipher(key_size);
    if (cipher == NULL) {
        return false;
    }
