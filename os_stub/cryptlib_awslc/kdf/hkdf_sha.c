...
/** @file
 * HMAC-SHA256/384/512 KDF Wrapper Implementation over AWS-LC.
 *
                                ...
                                size_t out_size)
{
    ...
    if (HKDF(out, out_size, md, key, key_size, salt, salt_size,
             info, info_size) != 1) {
        return false;
    }
                     ...
                     size_t prk_out_size)
{
    ...
    if (HKDF_extract(prk_out, &prk_out_size, md, key, key_size,
                     salt, salt_size) != 1) {
        return false;
    }
                    ...
                    size_t info_size, uint8_t *out, size_t out_size)
{
    ...
    if (HKDF_expand(out, out_size, md, prk, prk_size,
                    info, info_size) != 1) {
        return false;
    }
