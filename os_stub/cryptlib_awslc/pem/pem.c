...
/** @file
 * PEM (Privacy Enhanced Mail) format Handler Wrapper Implementation for AWS-LC.
 *
 * NOTE: AWS-LC supports PEM_read_bio_PrivateKey and PEM_read_bio_PUBKEY.
 * It does NOT have OSSL_PARAM / EVP_PKEY_get_utf8_string_param.
 * Key type verification uses EVP_PKEY_base_id / EVP_PKEY_id.
 **/
...
bool libspdm_sm2_get_private_key_from_pem(const uint8_t *pem_data,
                                          ...
                                          void **sm2_context)
{
    /* SM2 is not supported in AWS-LC */
    return false;
}
