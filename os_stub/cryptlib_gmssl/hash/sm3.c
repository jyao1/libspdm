...
void *sm3_256_new(void)
{
  ...
  sm3_ctx = allocate_pool (sizeof(SM3_CTX));
  return sm3_ctx;
}
...
boolean sm3_256_update(IN OUT void *sm3_context, IN const void *data,
		       IN uintn data_size)
{
  sm3_update (sm3_context, data, data_size);
  return TRUE;
}
...
{
  sm3_finish (sm3_context, hash_value);
  return TRUE;
}
...
{
  sm3_digest (data, data_size, hash_value);
  return TRUE;
}
