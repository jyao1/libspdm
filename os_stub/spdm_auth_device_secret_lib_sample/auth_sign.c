...
bool libspdm_auth_device_requester_data_sign(
    ...
    uint8_t *signature, size_t *sig_size)
{
    ...
    result = libspdm_get_requester_private_key_from_raw_data(base_asym_algo, &context);
    if (!result) {
        return false;
    }

    result = libspdm_auth_asym_sign(spdm_auth_version,
